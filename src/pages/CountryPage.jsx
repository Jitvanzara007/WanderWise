            ...
            {allPlaces.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 px-4 sm:px-6 lg:px-8">
                {allPlaces.map((place, index) => (
                  <div key={index} className="group bg-white rounded-xl overflow-hidden border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_16px_rgba(0,0,0,0.12)] transition-all duration-300 h-[400px]">
                    <div className="relative w-full h-[200px]">
                      ...
                      <div className="absolute bottom-0 left-0 w-full p-4">
                        <h3 className="text-lg font-bold text-white mb-1 line-clamp-1">{place.name}</h3>
                        <p className="text-sm text-gray-200 line-clamp-1">{place.cityName}</p>
                      </div>
                    </div>
                    <div className="p-4 bg-gradient-to-b from-white to-gray-50 h-[200px] flex flex-col">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center space-x-2 text-gray-500">
                          ...
                          <span className="text-sm">{place.duration || '2-3 hours'}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-4">
                        <Link 
                          to={`/places/${place.id}`}
                          className="inline-flex items-center text-amber-500 hover:text-amber-600 transition-colors text-sm font-medium"
                        >
                          View Details
                          <FiArrowRight className="ml-1.5 w-4 h-4" />
                        </Link>
                        <a 
                          href={`https://www.google.com/maps/search/${encodeURIComponent(place.name + ', ' + place.cityName)}`}
                          target="_blank"
